        ...
        if(count($categorias) == 0){
          ...
          foreach ($categorias as $categoria) {
			...
			$queryProd = "select m.id_menu, m.nome, m.descricao, m.preco, m.foto, false itemsozinho, true personalizacoesativas
					...
					--INNER JOIN estabelecimentos as e ON e.id_estabelecimento = m.id_estabelecimento 
					inner join empresas as emp on emp.id_empresa = m.id_empresa
					and REPLACE(LOWER(emp.nome), ' ', '') LIKE ? 
					group by m.id_menu, m.nome, m.descricao, m.preco, m.foto ";
			...
			$queryProd = "select m.id_menu, m.nome, m.descricao, m.preco, m.foto, false itemsozinho, true personalizacoesativas
             ...
             $queryProd = "SELECT itens.id_item, itens.nome, itens.descricao, itens.preco, itens.foto, 		itens.itemsozinho, itens.personalizacoesativas, coalesce(menus.id_menu,0) id_menu
					FROM itens 
					--INNER JOIN estabelecimentos as e ON e.id_estabelecimento = itens.id_estabelecimento 
					inner join empresas as emp on emp.id_empresa = itens.id_empresa
					INNER JOIN categorias ON categorias.id_categoria = itens.id_categoria 
					left join menus on menus.id_estabelecimento = e.id_estabelecimento
					WHERE itens.disponivel=true and REPLACE(LOWER(emp.nome), ' ', '') LIKE ? AND REPLACE(LOWER(categorias.nome), ' ', '') LIKE ? ";
				...
				<form method='POST'  enctype='multipart/form-data' action='' id='pedidoForm'>
					...
					if (strtolower($idCategoria) == 'menus') {
						$queryMenu = "select i.id_item, i.nome, i.descricao, i.preco, i.foto, i.itemsozinho, i.personalizacoesativas, m.id_menu
							...
							--INNER JOIN estabelecimentos as e ON e.id_estabelecimento = i.id_estabelecimento 
							inner join empresas as emp on emp.id_empresa = m.id_empresa
							INNER JOIN categorias ON categorias.id_categoria = i.id_categoria 
							and REPLACE(LOWER(emp.nome), ' ', '') LIKE LOWER(?) and m.id_menu=".$rowProd['id_menu'];
						...
						$queryMenu = "SELECT itens.id_item, itens.nome, itens.descricao, itens.preco, itens.foto, itemsozinho, personalizacoesativas, 0 id_menu
							...
							inner join empresas as emp on emp.id_empresa = itens.id_empresa
							INNER JOIN categorias ON categorias.id_categoria = itens.id_categoria 
							WHERE REPLACE(LOWER(emp.nome), ' ', '') LIKE LOWER(?) and itens.id_item=".$rowProd['id_item'];
