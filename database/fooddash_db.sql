...
(
    ...
    CHECK(
        ACTION_TYPE IN 
        ('ADDED ITEM','DELETED ITEM',
        'DELETED MENU', 'ADDED MENU',
        'ADDED CATEGORY','DELETED CATEGORY',
        'ADDED MENU ITEM', 'DELETED MENU ITEM')),
...
CREATE TABLE IF NOT EXISTS Pedidos (
    ...
    CHECK(estado
    IN ('EFETUADO','EM PREPARACAO',
    'A CAMINHO','FINALIZADO')),
...
CREATE OR REPLACE FUNCTION ADICIONAR_MENU()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (NEW.id_menu,NEW.id_estabelecimento,'ADDED MENU');
	RETURN NEW;
END;$$;

CREATE OR REPLACE FUNCTION REMOVER_MENU()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (old.id_menu,old.id_estabelecimento,'DELETED MENU');
	RETURN OLD;
END;$$;
...
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (NEW.id_item,NEW.id_estabelecimento,'ADDED ITEM');
	RETURN NEW;
END;$$;
...
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (old.id_item,old.id_estabelecimento,'DELETED ITEM');
	RETURN OLD;
END;$$;

CREATE OR REPLACE FUNCTION ADICIONAR_CATEGORIA()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (NEW.id_categoria,NEW.id_empresa,'ADDED CATEGORY');
	RETURN NEW;
END;$$;

CREATE OR REPLACE FUNCTION REMOVER_CATEGORIA()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (OLD.id_categoria,OLD.id_empresa,'DELETED CATEGORY');
	RETURN OLD;
END;$$;

CREATE OR REPLACE FUNCTION ADICIONAR_MENU_ITENS()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (NEW.id_item,NEW.id_menu,'ADDED MENU ITEM');
	RETURN NEW;
END;$$;

CREATE OR REPLACE FUNCTION REMOVER_MENU_ITENS()
RETURNS TRIGGER
LANGUAGE PLPGSQL AS $$
BEGIN
	INSERT INTO ACTION_LOGGER(AFFECTED_ID,AFFECTED_COMPANY,ACTION_TYPE) VALUES (OLD.id_item,OLD.id_menu,'DELETED MENU ITEM');
	RETURN OLD;
END;$$;

-- TRIGGERS
CREATE TRIGGER ADICIONAR_MENU AFTER INSERT ON Menus FOR EACH ROW
EXECUTE FUNCTION ADICIONAR_MENU();

CREATE TRIGGER REMOVER_MENU BEFORE DELETE ON Menus FOR EACH ROW
EXECUTE FUNCTION REMOVER_MENU();

CREATE TRIGGER ADICIONAR_ITEM AFTER INSERT ON Itens FOR EACH ROW
EXECUTE FUNCTION ADICIONAR_ITEM();

CREATE TRIGGER REMOVER_ITEM BEFORE DELETE ON Itens FOR EACH ROW
EXECUTE FUNCTION REMOVER_ITEM();

CREATE TRIGGER ADICIONAR_CATEGORIA AFTER INSERT ON Categorias FOR EACH ROW
EXECUTE FUNCTION ADICIONAR_CATEGORIA();

CREATE TRIGGER REMOVER_CATEGORIA BEFORE DELETE ON Categorias FOR EACH ROW
EXECUTE FUNCTION REMOVER_CATEGORIA();

CREATE TRIGGER ADICIONAR_MENU_ITENS AFTER INSERT ON Item_Menus FOR EACH ROW
EXECUTE FUNCTION ADICIONAR_MENU_ITENS();

CREATE TRIGGER REMOVER_MENU_ITENS BEFORE DELETE ON Item_Menus FOR EACH ROW
EXECUTE FUNCTION REMOVER_MENU_ITENS();